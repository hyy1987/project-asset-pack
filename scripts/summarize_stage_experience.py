...
def main() -> int:
    ...
    return invoke_agent_skill(
        ...
        [
            ...
            f"Stage title: {title}",
            f"Stage report: {repo_relative(out_dir / 'stage-report.md')}",
            f"Quality gate: {repo_relative(out_dir / 'quality-gate.md')}",
            f"Stage review: {repo_relative(stage_reviewed_dir(args.project, args.stage_id) / 'stage-review.md')}",
